...
const Track = () => {
  ...
  const validateOrderId = (id) => {
    ...
    if (id.length === 8 && uppercaseLetters.length >= 3) {
      ...
      setError("Invalid order ID. Ensure your tracking id is correct.");
      return false;
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-primary to-secondary dark:bg-gradient-to-b dark:from-darkBackground dark:to-darkSecondary text-white p-4">
      <header className={`fixed top-0 left-0 w-full z-50 bg-white dark:bg-darkSecondary dark:text-white shadow-lg transition-all duration-500 ease-in-out transform bg-opacity-90`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
          ...
          <nav className="flex space-x-4 items-center">
            ...
            <Link href="/products" className="text-gray-700 hover:text-primary dark:text-white transition-colors transform hover:scale-105 duration-200">Product</Link>
            <DarkModeToggle 
              visible={true}
            />
          </nav>
      ...
      <div className="max-w-3xl w-full bg-white dark:bg-darkBackground text-black dark:text-white rounded-lg shadow-lg p-6 space-y-6">
        <h2 className="text-3xl font-bold text-center text-primary dark:text-darkAccent">
          Track Your Order
        </h2>

        {/* Order ID Input */}
        <div>
          <label className="block text-lg font-medium mb-4 text-secondary dark:text-darkAccent">
            Enter Your Order ID
          </label>
          <div className="flex justify-center space-x-2">
            {Array.from({ length: 8 }).map((_, index) => (
              <input
                ...
                className={`w-12 h-12 border dark:border-gray-600 rounded-md text-center text-lg text-primary dark:text-darkAccent font-semibold shadow-md focus:outline-none focus:ring-2 ${
                  error
                    ? "border-red-500 focus:ring-red-500"
                    : "focus:ring-primary dark:focus:ring-darkPrimary"
                }`}
              />
            ))}
          </div>
        ...
        <div className="text-center">
          <button
            ...
            className={`w-full max-w-sm py-3 text-lg font-semibold rounded-lg shadow-md transition-all duration-300 ${
              !!error || orderId.length !== 8
                ? "bg-gray-400 cursor-not-allowed"
                : "bg-primary text-white hover:bg-secondary dark:bg-darkAccent"
            }`}
          >
            Track Order
          </button>
      ...
      {/* Tracking Result */}
      {trackingResult && (
        <div className="mt-12 w-full max-w-3xl bg-white dark:bg-darkSecondary text-black dark:text-white rounded-lg shadow-lg p-6 space-y-4">
          <h3 className="text-2xl font-bold border-b border-gray-300 dark:border-gray-600 pb-2">
            Order Details
          </h3>
          <p>
